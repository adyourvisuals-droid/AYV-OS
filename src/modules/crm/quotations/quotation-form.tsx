...
export function QuotationForm({
  ...
  const defaultItems: LineItem[] = [
    { name: "Service package", description: "", quantity: 1, unitPrice: 0 },
  ];
    ...
    <form action={action} className="flex max-w-3xl flex-col gap-5">
      <div className="grid grid-cols-2 gap-4">
        <Field label="Title" htmlFor="title" className="col-span-2">
          <Input id="title" name="title" placeholder="Website Redesign Quotation" required />
        </Field>
      ...
      <Field label="Line items" htmlFor="itemsJson">
        <LineItemsEditor name="itemsJson" defaultItems={defaultItems} taxPercent={taxPercent} />
      </Field>
      <div>
